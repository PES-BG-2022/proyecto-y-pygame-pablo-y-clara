...
while not game_over:
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			# Jugador 1
			if event.key == pygame.K_w: #sube con la letra w
				player1_y_speed = -3
			if event.key == pygame.K_s: #baja con la letra s
				player1_y_speed = 3
			# Jugador 2
			if event.key == pygame.K_UP: #sube con la flecha arriba
				player2_y_speed = -3
			if event.key == pygame.K_DOWN: #sube con la flecha abajo
				player2_y_speed = 3
	...
	pelota = pygame.draw.circle(screen, rojo, (pelota_x, pelota_y), 10)
